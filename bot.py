...
class TradingBotGUI:
    ...
    def add_equity(self):
        ...
        level_prices = {i+1: round(entry_price * (1 - drawdown * (i + 1)), 2) 
    ...
    def place_order(self,symbol,price,level):
        if price is None or price <= 0:
            return  # prevents “limit price must be > 0”
        if -level in self.equities[symbol]["levels"] or -1 in self.equities[symbol]["levels"]:
            return
        try: 
            api.submit_order(
                symbol=symbol,
            ...
            self.equities[symbol]["levels"][f"filled_{level}"] = price
